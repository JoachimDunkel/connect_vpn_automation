...
def read_credentials(connection_backend):
    ...
    with open(str(res.PATH_CREDENTIALS_FILE), 'r') as stream:
        try:
            ...
            connection_backend.config.VPN_PUB_IP = _ensure_config_value_provided(credentials['VPN_PUB_IP'],
                                                                                 "Please provide the expected ipv4 address of your desired vpn to: VPN_PUB_IP")
            connection_backend.config.OPENVPN_SCRIPT_PATH = _ensure_config_value_provided(
                credentials['OPENVPN_SCRIPT_PATH'],
                "Please provide a correct path to your openvpn-script to: OPENVPN_SCRIPT_PATH")
            connection_backend.config.SUDO_PW = _ensure_config_value_provided(credentials['SUDO_PW'],
                                                                              "Please provide a correct sudo-pw to: SUDO_PW")
            connection_backend.config.USER_NAME = _ensure_config_value_provided(credentials['USER_NAME'],
                                                                                "Please provide the correct username to your openvpn login to: USER_NAME")
            connection_backend.config.USER_PW = _ensure_config_value_provided(credentials['USER_PW'],
                                                                              "Please provide the correct password to your openvpn login to: USER_PW")
